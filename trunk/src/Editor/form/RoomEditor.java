 ...
 */
public class RoomEditor extends javax.swing.JDialog {
    ...
    public RoomEditor(FaustEditor editor, ResourceDelegate delegate, Room room, boolean modal) {
    ...
    private void init() {
        ...
        //
        final boolean exists = delegate.getInstanceCount(room) >= 1;

        // Check to see if the world even exists
        setTitle(exists ? "Room Editor: " + room.getDisplayName() : "Create a new Room");

        // Displays default cell size only if the world cell doesn't exist
        widthJField.setValue(exists ? room.getWidth() : 640);
        heightJField.setValue(exists ? room.getHeight() : 480);
    ...
    private void setupManifestBinder() {
        ...
        // Testing it out.
        binder = new ManifestBinder(delegate, room);
        ...
        binder.setEdit(delegate.getInstanceCount(room) >= 1);
    ...
    private void finish() {
        ...
        if (box_delegate.isSelected()) {
            ...
            room.setWidth(((Number) widthJField.getValue()).intValue());
            room.setHeight(((Number) heightJField.getValue()).intValue());

            // Apply these changes to the fMap
            room.setReferenceID(field_reference.getText());
            room.setReferenceName(field_name.getText());
            room.setDisplayName(field_display.getText());
            ...
            //
            ResourceWriter.write(delegate, room);

            //
            delegate.addResource(room);
