 ...
 */
public class WorldCellLayerEditor extends javax.swing.JDialog {

    // Variable Declaration
    // Project Classes
    private FaustEditor editor;
    private WorldCellLayer layer;
    ...
    public WorldCellLayerEditor(FaustEditor editor, ResourceDelegate delegate, WorldCell worldCell, WorldCellLayer layer, boolean edit, boolean modal) {
    ...
    private void init() {
        ...
        if (layer == null) {
            ...
            layer = new WorldCellLayer(worldCell, "");
        } else {
        ...
        // Grab all the worlds
        final Object[] worldCells = delegate.getType(WorldCell.class);

        // Iterate
        for (int i = 0; i < worldCells.length; i++) {

            // Cast to an FWorld
            final WorldCell current = (WorldCell) worldCells[i];
    ...
    private void finish() {
        ...
        if (validJCheckBox.isSelected()) {
            ...
            // Map must exist
            if (worldCell != null) {
                worldCell.addWorldCellLayer(worldCell.getWorld(), layer);
            }
